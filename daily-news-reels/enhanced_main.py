...
def generate_gender_aware_audio(script: str, output_path: str, category: str = 'general') -> bool:
    ...
    try:
        # Ensure we have the voice system
        if not VOICE_SYSTEM:
        ...
        try:
            ...
            logger.error(f"gTTS generation failed: {tts_error}")
            return False
...
Examples:
  ...
  python enhanced_main_gender_aware.py --max-articles 20 --test         # Quick test with variety
    ...
    parser.add_argument('--max-workers',
                       type=int,
                       default=4,
                       help='Maximum parallel workers for processing (default: 4)')
    
    parser.add_argument('--audio-workers', 
                       type=int,
                       default=4,
                       help='Parallel workers for audio generation (default: 4)')
    
    parser.add_argument('--video-workers',
                       type=int,
                       default=2,
                       help='Parallel workers for video generation (default: 2)')
...
def main():
    ...
    try:
        ...
        if hasattr(pipeline, 'audio_service'):
            original_generate = pipeline.audio_service.generate_audio if hasattr(pipeline.audio_service, 'generate_audio') else None
            
            def patched_generate_audio(script, output_path, **kwargs):
                category = kwargs.get('category', 'general')
                success = generate_gender_aware_audio(script, output_path, category)
                if not success and original_generate:
                    # Fallback to original method if gender-aware fails
                    return original_generate(script, output_path, **kwargs)
                return success
            ...
            logger.warning("⚠️ Audio service not found, using fallback method")
        ...
        logger.info(f"🔧 Parallel workers: Content={args.max_workers}, Audio={args.audio_workers}, Video={args.video_workers}")
        
        # Execute pipeline
        if args.test:
            logger.info("🧪 Running quick test mode with parallel processing...")
            stats, articles = pipeline.execute_quick_test(max_articles=2)
        else:
            logger.info("🏃 Running full pipeline with parallel male/female voice selection...")
            
            # Add parallel processing configuration
            pipeline.content_processor.max_workers = args.max_workers
            pipeline.audio_service.max_workers = args.audio_workers  
            pipeline.video_service.max_workers = args.video_workers
